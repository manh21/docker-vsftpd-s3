...
PASV_ADDRESS=${PASV_ADDRESS:-$(timeout 1 wget -qO- https://api.ipify.org 2>/dev/null ||:)}
PASV_MIN_PORT=${PASV_MIN_PORT:-65000}
PASV_MAX_PORT=${PASV_MAX_PORT:-65000}

# VSFTPD Banner
FTPD_BANNER=${FTPD_BANNER:-Komodo FTP Server}
...
echo "${FTPD_USERS}" |sed 's/ /\n/g' |while read line; do
  echo ${line//::/ } |while read ftpd_user ftpd_pass s3_bucket aws_access_key_id aws_secret_access_key; do
    ...
    # Mount s3fs
    /usr/bin/s3fs ${s3_bucket:-$S3_BUCKET} /home/${ftpd_user} -o url=${S3_ENDPOINT} -o nosuid,nonempty,nodev,allow_other,complement_stat,mp_umask=027,uid=$(id -u ${ftpd_user}),gid=$(id -g ${ftpd_user}),passwd_file=/home/${ftpd_user}/.passwd-s3fs,default_acl=${S3_ACL},retries=5
